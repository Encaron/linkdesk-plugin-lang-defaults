# 官方语言（lang-defaults）——LinkDesk 插件仓

> **本文件是给在这个仓里干活的 AI 看的**（Claude Code / Codex / Cursor / …）。人看 `README.md`。
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`lang-defaults`）。当前版本 `1.0.20`。

## 1. 这是什么

官方语言包——中文 / English 两份词典。**纯数据插件，没有源码。**

**用户在哪看到它**：不出现在任何区域。它的作用是让 `t()` 有译文。
